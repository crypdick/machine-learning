...
def plot_curve():
    ...
    training_sizes, training_scores, testing_scores = learning_curve.learning_curve(reg, X, y, cv=cv, scoring=scorer,
                                                                                    train_sizes=size)
    ...
    print type(X)
    print type(y)
    print type(training_scores)
    # Plot aesthetics
    plt.ylim(-0.1, 1.1)
    plt.ylabel("Curve Score")
    plt.xlabel("Training Points")
    plt.legend(bbox_to_anchor=(1.1, 1.1))
    plt.plot(X, testing_scores)
    plt.plot(X, training_scores)
    plt.show()